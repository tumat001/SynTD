...
class LineDrawParams:
	...
	var func_name__beam_reach_dest_pos : String   # accepts 1 arg, "self" is passed
	
	var _called_beam_reached_dest_pos : bool = false
...
func _process(delta):
	for param in _all_line_draw_params:
		...
		if !param._called_beam_reached_dest_pos and param.current_line_start_length >= param._length_from_source_to_dest_pos:
			if param.func_source != null:
				param.func_source.call(param.func_name__beam_reach_dest_pos, param)
