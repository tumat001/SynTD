...
func _ready():
	...
	range_module.add_targeting_option(Targeting.PERCENT_EXECUTE)
	range_module.set_current_targeting(Targeting.PERCENT_EXECUTE)
...
func _heal_ability_activated():
	var targets = range_module.get_targets(2)
