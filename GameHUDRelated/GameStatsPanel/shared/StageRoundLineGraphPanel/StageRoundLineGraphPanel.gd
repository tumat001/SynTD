...
func set_stage_rounds_col_label_to_point_val_map(line_label_to_line_color_map : Dictionary, stage_round_data_point_arr : Array, total_data_points_count : int):
	graph.limit_x_count = total_data_points_count - 1
	graph.max_value = 150 #TODO CONTINUE THIS. Set this val based on given data..
...
func _set_stage_rounds_coll_deferred(line_label_to_line_color_map : Dictionary, stage_round_data_point_arr : Array):
	...
	for data_point in stage_round_data_point_arr:
		...
		data_point_col_label # other_data. can be anything
		)
