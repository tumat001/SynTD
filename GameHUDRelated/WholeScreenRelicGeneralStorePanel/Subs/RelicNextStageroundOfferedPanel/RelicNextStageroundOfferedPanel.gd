...
func set_stage_round_manager(arg_manager):
	...
	stage_round_manager.connect("current_stageround", self, "_on_round_ended", [], CONNECT_PERSIST)


func _on_round_ended(arg_stageround : Stageround):
	if _next_stageround_id_for_relic_offer == arg_stageround.id and !_no_more_left:
		_update_next_stageround_offered_display()
...
func _update_next_stageround_offered_display():
	...
	if next_relic_stageround != null:
		var stage_and_round : Array = Stageround.convert_stageround_id_to_stage_and_round_num(next_relic_stageround)
